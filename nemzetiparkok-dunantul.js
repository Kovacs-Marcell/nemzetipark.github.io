function start() {
    ...
    mutasd(kezdoKep);
    var kepElemek = document.getElementsByClassName('allat-kep');
    for (var i = 0; i < kepElemek.length; i++) {
        kepElemek[i].addEventListener('mouseover', function() {
            mutasd(this);
        });
    }
}
...
function mutasd(element) {
    document.getElementById('kiemelt-kep').style.backgroundImage = "url(" + element.getAttribute('src') + ")";
    document.getElementById('kiemelt-nev').innerText = element.getAttribute('alt');
